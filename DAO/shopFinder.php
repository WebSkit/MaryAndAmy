	...
	class shopFinder{
		...
		function findValidShops($userPostCode,$userAcceptedDistance)
		{
			...
			while($row=$result->fetch_assoc())
			{
				...
				if($distance<=$userAcceptedDistance && $distance<=$row["served_area"])
				{
					...
					$validCompaniesArray[$tempCount]=array("id"=>$row["baker_id"],"companyName"=>$row["companyName"],"addressLine1"=>$row["addressLine1"],"addressLine2"=>$row["addressLine2"],"postcode"=>$row["postcode"], "distance"=>$distance,"reviewScore"=>$reviewAverage);
